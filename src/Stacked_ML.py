...
Stacked features: CodeBERT embeddings (from cache) + hand-crafted (Stylometry / Structural)
...
HC_CACHE_DIR   = "cache/stylometry"
MAX_LENGTH     = 512        # must match Embedding_ML.py setting
NORMALIZE_EMBS = True       # must match Embedding_ML.py setting

os.makedirs(HC_CACHE_DIR, exist_ok=True)


# ─────────────────────────────────────────────
# 2. CodeBERT cache path helper (mirrors Embedding_ML.py)
# ─────────────────────────────────────────────
def _emb_cache_path(cache_key: str) -> str:
    cfg_hash = hashlib.md5(
        json.dumps({"max_length": MAX_LENGTH, "normalize": NORMALIZE_EMBS},
                   sort_keys=True).encode()
    ).hexdigest()[:8]
    return os.path.join(EMB_CACHE_DIR, cache_key, "{split}_" + cfg_hash + ".npy")


def load_embeddings(cache_key: str, split: str) -> np.ndarray:
    path = _emb_cache_path(cache_key).format(split=split)
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Embedding cache not found: {path}\n"
            "Run Embedding_ML.py first to generate CodeBERT embeddings."
        )
    emb = np.load(path)
    print(f"  [cache] {split:5s} CodeBERT  {path}  shape={emb.shape}")
    return emb


# ─────────────────────────────────────────────
# 3. Hand-crafted feature extractors (same as Stylometry_ML.py)
# ─────────────────────────────────────────────
...
def _build_matrix(corpus: list[str], extractor, tag: str):
    ...
    cache_path = os.path.join(HC_CACHE_DIR, f"{tag}_{key}.pkl")
...
# ─────────────────────────────────────────────
# 5. Load CodeBERT embeddings from cache
# ─────────────────────────────────────────────
print("\nLoading CodeBERT embeddings from cache...")
train_emb = load_embeddings("codebert", "train")
val_emb   = load_embeddings("codebert", "val")
test_emb  = load_embeddings("codebert", "test")
pub_emb   = load_embeddings("codebert", "pub")


# ─────────────────────────────────────────────
# 6. Extract hand-crafted features
# ─────────────────────────────────────────────
...
X_train_stylo_sc = scaler_stylo.fit_transform(X_train_stylo)
X_val_stylo_sc   = scaler_stylo.transform(X_val_stylo)
X_test_stylo_sc  = scaler_stylo.transform(X_test_stylo)
X_pub_stylo_sc   = scaler_stylo.transform(X_pub_stylo)

scaler_ast = StandardScaler()
X_train_ast_sc = scaler_ast.fit_transform(X_train_ast)
X_val_ast_sc   = scaler_ast.transform(X_val_ast)
X_test_ast_sc  = scaler_ast.transform(X_test_ast)
X_pub_ast_sc   = scaler_ast.transform(X_pub_ast)

X_train_hc_sc = np.hstack([X_train_stylo_sc, X_train_ast_sc])
X_val_hc_sc   = np.hstack([X_val_stylo_sc,   X_val_ast_sc])
X_test_hc_sc  = np.hstack([X_test_stylo_sc,  X_test_ast_sc])
X_pub_hc_sc   = np.hstack([X_pub_stylo_sc,   X_pub_ast_sc])
...
# ─────────────────────────────────────────────
# 8. Feature sets × Classifiers
# ─────────────────────────────────────────────
FEATURE_SETS = {
    "CodeBERT+Stylometry": (
        np.hstack([train_emb, X_train_stylo_sc]),
        np.hstack([val_emb,   X_val_stylo_sc]),
        np.hstack([test_emb,  X_test_stylo_sc]),
        np.hstack([pub_emb,   X_pub_stylo_sc]),
    ),
    "CodeBERT+Structural": (
        np.hstack([train_emb, X_train_ast_sc]),
        np.hstack([val_emb,   X_val_ast_sc]),
        np.hstack([test_emb,  X_test_ast_sc]),
        np.hstack([pub_emb,   X_pub_ast_sc]),
    ),
    "CodeBERT+HandCrafted": (
        np.hstack([train_emb, X_train_hc_sc]),
        np.hstack([val_emb,   X_val_hc_sc]),
        np.hstack([test_emb,  X_test_hc_sc]),
        np.hstack([pub_emb,   X_pub_hc_sc]),
    ),
}
...
for feat_name, (Xtr, Xv, Xte, Xpu) in FEATURE_SETS.items():
    print(f"\n{'#'*55}")
    print(f"  Feature set: {feat_name}  (dim={Xtr.shape[1]})")
    print(f"{'#'*55}")
    ...
    for clf_name, clf_proto in CLASSIFIERS.items():
        ...
        safe_name = f"{feat_name.lower().replace('+','_').replace(' ','_')}_{clf_name.lower()}"
...
print("  SUMMARY")
print(f"{'='*80}")
print(f"  {'Features':<26} {'Classifier':<22} {'Val Acc':>8} {'Val F1':>8} {'Pub Acc':>8} {'Pub F1':>8}")
print(f"  {'-'*26} {'-'*22} {'-'*8} {'-'*8} {'-'*8} {'-'*8}")
for r in sorted_results:
    print(f"  {r['features']:<26} {r['classifier']:<22} {r['val_acc']:>8.4f} {r['val_f1']:>8.4f} {r['pub_acc']:>8.4f} {r['pub_f1']:>8.4f}")
print(f"{'='*80}\n")
...
md_lines = [
    ...
    "",
    "## Configuration",
    f"- **Embedding model**: CodeBERT (microsoft/codebert-base, from cache)",
